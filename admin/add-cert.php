...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($issuer) || empty($cert_date) || empty($category)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO certifications (title, issuer, cert_date, category) VALUES (?, ?, ?, ?)");
        $stmt->execute([$title, $issuer, $cert_date, $category]);
...
<body>
    <div class="admin-container">
        <h1 class="page-title" style="margin-bottom: 24px;">Add New Certification</h1>
        
        <?php if ($error): ?>
            <div class="error-msg"><?php echo $error; ?></div>
        <?php endif; ?>

        <form action="add-cert.php" method="POST">
            <div class="form-group">
                <label>Certification Title</label>
                <input type="text" name="title" class="form-input" placeholder="e.g., AWS Cloud Practitioner">
            </div>
            
            <div class="form-group">
                <label>Issuer / Organization</label>
                <input type="text" name="issuer" class="form-input" placeholder="e.g., Amazon Web Services">
            </div>
            
            <div class="form-group">
                <label>Date Issued</label>
                <input type="text" name="cert_date" class="form-input" placeholder="e.g., March 2026">
            </div>
            
            <div class="form-group">
                <label>Category</label>
                <input type="text" name="category" class="form-input" placeholder="e.g., Cloud Computing">
            </div>
